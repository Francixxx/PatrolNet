...
const UserProfileModal = ({ isOpen, onClose, userProfile, onSave }) => {
  ...
  useEffect(() => {
    if (userProfile) {
      setFormData({
        image: userProfile.IMAGE ? `http://192.168.1.149:3001/uploads/${userProfile.IMAGE}` : '',
        imageFile: null,
        fullName: userProfile.NAME || '',
        username: userProfile.USERNAME || localStorage.getItem('username') || '',
        password: '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.1.149:3001/api/user/${username}`, {
        method: 'PUT',
        body: formDataToSend,
      });
...
const Navbar = ({ currentUser }) => {
  ...
  const fetchUserProfile = async () => {
    ...
    if (username) {
      try {
        const url = `http://192.168.1.149:3001/api/user/${username}`;
  ...
  const getAvatarSrc = () => {
    ...
    if (storedImage && storedImage.trim() !== '') {
      ...
      return `http://192.168.1.149:3001/uploads/${storedImage}`;
    }
    
    if (userProfile && userProfile.IMAGE && userProfile.IMAGE.trim() !== '') {
      console.log('Using profile image:', userProfile.IMAGE);
      return `http://192.168.1.149:3001/uploads/${userProfile.IMAGE}`;
    }
