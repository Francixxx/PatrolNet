...
const Notifications: React.FC = () => {
  ...
  const fetchAssignedIncidents = async () => {
    try {
      const response = await axios.get(`http://10.170.82.215:3001/api/incidents/assigned/${username}`);
  ...
  const fetchReportedIncidents = async () => {
    try {
      const response = await axios.get(`http://10.170.82.215:3001/api/incidents/reported/${username}`);
      const fetchedIncidents = response.data || [];
  ...
  const resolveIncident = async (incidentId: number) => {
    try {
      await axios.put(`http://10.170.82.215:3001/api/incidents/${incidentId}/resolve`, {
