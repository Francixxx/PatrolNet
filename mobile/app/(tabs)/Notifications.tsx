...
const Notifications: React.FC = () => {
  ...
  const fetchReportedIncidents = async () => {
    try {
      const response = await axios.get(`http://192.168.1.149:3001/api/incidents/reported/${username}`);
      const fetchedIncidents = response.data || [];
  ...
  const resolveIncident = async (incidentId: number) => {
    try {
      await axios.put(`http://192.168.1.149:3001/api/incidents/${incidentId}/resolve`, {
